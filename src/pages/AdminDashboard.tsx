...
export default function AdminDashboard() {
  ...
  const deleteAllOrders = async () => {
    if (window.confirm('Tem certeza que deseja excluir TODOS os pedidos? Esta ação não pode ser desfeita.')) {
      setIsDeleting(true);
      try {
        // With CASCADE constraint, we only need to delete from orders table
        const { error } = await supabase
          .from('orders')
          .delete()
          .not('id', 'is', null);
